...
export async function getUserRole(userId: string) {
    const [{ data: menteeData }, { data: mentorData }, { data: adminData }] = await Promise.all([
        adminSupabase.from("MENTEE_GROUPS").select("role").eq("id", userId).maybeSingle(),
        adminSupabase.from("mentor").select("role, is_admin").eq("id", userId).maybeSingle(),
        adminSupabase.from("admin").select("role").eq("id", userId).maybeSingle()
    ])
